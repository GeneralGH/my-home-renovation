...
import woodworkingApi from '@/api/woodworking'
export default {
  ...
  methods: {
    initList() {
      this.data = []
      this.$request.get(woodworkingApi.getListUrl)
      .then(res => {
    ...
    addSubmit(e) {
      ...
      this.$request.post(woodworkingApi.addUrl, obj)
      .then(res => {
    ...
    updateSubmit(e) {
      let obj = JSON.parse(e)
      this.$request.put(woodworkingApi.updateUrl, obj)
      .then(res => {
        this.$refs.BaseList.templateShow = false
        this.$message.success('保存成功')
        this.initList()
    ...
    delSubmit(id) {
      this.$request.delete(woodworkingApi.delUrl + id)
      .then(res => {
