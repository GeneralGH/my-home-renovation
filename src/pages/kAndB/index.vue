...
import kandbApi from '@/api/kandb'
export default {
  ...
  methods: {
    initList() {
      this.data = []
      this.$request.get(kandbApi.getListUrl)
      .then(res => {
    ...
    addSubmit(e) {
      ...
      this.$request.post(kandbApi.addUrl, obj)
      .then(res => {
    ...
    updateSubmit(e) {
      let obj = JSON.parse(e)
      this.$request.put(kandbApi.updateUrl, obj)
      .then(res => {
        this.$refs.BaseList.templateShow = false
        this.$message.success('保存成功')
        this.initList()
    ...
    delSubmit(id) {
      this.$request.delete(kandbApi.delUrl + id)
      .then(res => {
