...
import oilerApi from '@/api/oiler'
export default {
  ...
  methods: {
    initList() {
      this.data = []
      this.$request.get(oilerApi.getListUrl)
      .then(res => {
    ...
    addSubmit(e) {
      ...
      this.$request.post(oilerApi.addUrl, obj)
      .then(res => {
    ...
    updateSubmit(e) {
      let obj = JSON.parse(e)
      this.$request.put(oilerApi.updateUrl, obj)
      .then(res => {
        this.$refs.BaseList.templateShow = false
        this.$message.success('保存成功')
        this.initList()
    ...
    delSubmit(id) {
      this.$request.delete(oilerApi.delUrl + id)
      .then(res => {
